...
class ReplayBicycle:
    ...
    def run_jpeg(self, lateral_shift=4, starting_meters=60, start_steering_angle=None):
        ...
        for _ in range(5):
            ...
            """
            def blurring_defense(img):
                rgb = yuv2rgb(img.reshape(IMG_INPUT_SHAPE)[0].transpose((1, 2, 0))) / 255
                rgb = self.magnet_model.predict(np.expand_dims(rgb, axis=0))[0] * 255
                yuv = rgb2yuv(rgb.astype(np.uint8))
                return yuv.transpose((2, 0, 1)).flatten()

            np.save('before', np.vstack(model_img_inputs))
            model_img_inputs = [blurring_defense(a) for a in model_img_inputs]
            np.save('after', np.vstack(model_img_inputs))
            model_rnn_inputs = []
            model_outputs = []
            rnn_input = np.zeros(RNN_INPUT_SHAPE)
            desire_input = np.zeros(MODEL_DESIRE_INPUT_SHAPE)
