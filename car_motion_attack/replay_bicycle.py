...
class ReplayBicycle:
    ...
    def run_noise(self, lateral_shift=4, starting_meters=60, start_steering_angle=None):
        logger.debug("enter")
        # initialize car model
        self.car_motion.setup_masks(
            lateral_shift=lateral_shift, starting_meters=starting_meters
        ...
        for _ in range(5):
            ...
            def noise_defense(img):
                _img = (img + np.random.normal(0, 0.05, img.shape)).clip(YUV_MIN, YUV_MAX)
                return _img

            model_img_inputs = [noise_defense(a) for a in model_img_inputs]
        ...
        np.save('benign_model_in_aaa', np.stack(model_img_inputs))
        logger.debug("exit")
        return self.car_motion
