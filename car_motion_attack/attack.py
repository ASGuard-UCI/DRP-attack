...
class CarMotionAttack:
    ...
    def replay(
        ...
        for _ in range(5):
            ...
            def jpeg_defense(img, patch):

                _img = np.where(np.isnan(patch), img.reshape(IMG_INPUT_SHAPE)[0].transpose((1, 2, 0)), patch)
                rgb = yuv2rgb(_img).astype(np.uint8)
                tmp = io.BytesIO()
                Image.fromarray(rgb).save(tmp, format='jpeg', quality=20)
            ...
            tmp = [
                jpeg_defense(model_img_inputs[i], list_patches[i])
                for i in range(self.n_frames)
            ]
